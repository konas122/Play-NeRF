...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, skips=[4], output_color_ch=3):
...
class DirectTemporalNeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, input_ch_time=1,
                 skips=[4], embed_fn=None, zero_canonical=True):
        ...
        self._occ = NeRF(D=D, W=W, input_ch=input_ch,
                         input_ch_views=input_ch_views,
                         skips=skips, output_color_ch=3)
    ...
    def forward(self, x, ts):
        ...
        out, _ = self._occ(torch.cat([input_pts, input_views], dim=-1))
        return out, dx
